...
if(isset($_POST['add'])){
    ...
    $query = mysqli_query($con,"UPDATE `new_data` set `date`='$date', `unit`='$unit', `po_provider`='$po_provider', `po_number`='$po_number', `bill_qty`='$bill_qty', `goods_description`='$goods_description', `po_value`='$po_value', `po_unit_price`='$po_unit_price', `po_costing`='$po_costing', `profit`='$profit', `challan_number`='$challan_number', `pi_number`='$pi_number', `received_lc`='$received_lc' where new_data_id = '$id'");
                        ...
                        <div class="card-body">
                            <div class="digital-add needs-validation row">
                               ...
                               <div class="form-group col-md-6">
                                ...
                                <input class="form-control" id="date" value="<?=$details['date']?>" type="date" required="" name="date">
                            </div>
                            <div class="form-group col-md-6">
                                <label for="unit" class="col-form-label pt-0"><span>*</span> Buisness Unit</label>
                                <input class="form-control" id="unit" type="text" value="<?=$details['unit']?>" name="unit" required>
                            </div>
                            <div class="form-group col-md-6">
                                ...
                                <select class="form-control select2" id="po_provider"  name="po_provider" required >
                                   ...
                                   while($row = mysqli_fetch_assoc($cat)){ 
                                    ?>
                                    <option <?php if($details['po_provider']==$row['po_provider_id']){ echo "selected"; }?> value="<?=$row['po_provider_id'];?>"><?=$row['provier_name'];?></option>
                                <?php } ?>
                            </select>
                        </div>
                        <div class="form-group col-md-6">
                            <label for="po_number" class="col-form-label pt-0"><span></span>Po Number (WFX)</label>
                            <input class="form-control" id="po_number"  value="<?=$details['po_number']?>" type="text" name="po_number" required>
                        </div>

                        <div class="form-group col-md-6">
                            <label for="bill_qty" class="col-form-label"><span>*</span>Billing Quantity</label>
                            <input class="form-control" id="bill_qty" type="text"  value="<?=$details['bill_qty']?>" name="bill_qty" onkeyup="calculate_profit()">
                        </div>
                        <div class="form-group col-md-6">
                            <label for="goods_description" class="col-form-label"><span>*</span>Goods Description</label>
                            <textarea class="form-control" id="goods_description" name="goods_description"><?=$details['goods_description'];?></textarea>
                        </div>
                        <div class="form-group col-md-6">
                            <label for="po_value" class="col-form-label"><span>*</span>Po Value</label>
                            <input class="form-control" id="po_value" type="text" value="<?=$details['po_value'];?>" required="" name="po_value">
                        </div>

                        <div class="form-group col-md-6">
                            <label for="po_unit_price" class="col-form-label"><span>*</span>Po unit Price</label>
                            <input class="form-control" id="po_unit_price" type="text" value="<?=$details['po_unit_price'];?>" required="" name="po_unit_price" onkeyup="calculate_profit()">
                        </div>

                        <div class="form-group col-md-6">
                            <label for="po_costing" class="col-form-label"><span>*</span>Po Costing</label>
                            <input class="form-control" id="po_costing" value="<?=$details['po_costing'];?>" type="text" required="" name="po_costing" onkeyup="calculate_profit()">
                        </div>
                        <div class="form-group col-md-6">
                            <label for="profit" class="col-form-label"><span>*</span>Profit <span id="detail_data"></span></label>
                            <input class="form-control" id="profit" value="<?=$details['profit'];?>" type="text" required=""  name="profit">
                        </div>
                        <div class="form-group col-md-6">
                            <label for="challan_number" class="col-form-label"><span></span>Challan Number</label>
                            <input class="form-control" id="challan_number" value="<?=$details['challan_number'];?>" type="text"  name="challan_number">
                        </div>
                        <div class="form-group col-md-6">
                            <label for="pi_number" class="col-form-label"><span></span>Pi Number</label>
                            <input class="form-control" id="pi_number" type="text" value="<?=$details['pi_number'];?>" name="pi_number" >
                        </div>
                        <div class="form-group col-md-6">
                            <label for="received_lc" class="col-form-label"><span></span>Received Lc</label>
                            <input class="form-control" id="received_lc" value="<?=$details['received_lc'];?>" type="text" name="received_lc">
                        </div>
